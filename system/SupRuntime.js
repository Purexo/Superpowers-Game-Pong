...
var Player = (function () {
    ...
    Player.prototype._loadManifest = function (callback) {
        var _this = this;
        this.getAssetData("game.json", "json", function (err, gameData) {
            if (err != null) {
                callback(new Error("Failed to load game manifest"));
                return;
            }
            _this.gameName = gameData.name;
            ...
            for (var _i = 0, _a = gameData.assets; _i < _a.length; _i++) {
                var asset = _a[_i];
                walk(asset);
            }
            callback();
    ...
    Player.prototype._loadAssets = function (progressCallback, callback) {
        ...
        var onAssetLoaded = function (err, entry, asset) {
            ...
            _this._assetsById[entry.id] = asset;
            progressCallback();
            assetsLoaded++;
    ...
    Player.prototype.getAssetData = function (path, responseType, callback) {
        ...
        xhr.onload = function (event) {
            ...
            if (xhr.status !== 200 && xhr.status !== 0) {
                callback(new Error("Could not get " + path));
                return;
            }
