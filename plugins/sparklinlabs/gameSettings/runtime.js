...
function lateStart(player, callback) {
    var scene = player.resources.gameSettings.startupScene;
    if (scene != null) {
        var entry = player.entriesByPath[scene];
        if (entry == null) {
            callback(new Error("Startup scene not found: " + scene));
            return;
        }
        var outerAsset = player.getOuterAsset(entry.id);
        if (outerAsset.type !== "scene") {
            callback(new Error("Startup scene not found: " + scene));
            return;
        }
        window.Sup.loadScene(scene);
    }
    callback();
}
